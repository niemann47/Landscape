...
void Object::init(shaderloc &prog) {
    ...
    if (colorAttributes.texSize == 1200) {
        ...
        for( int i = 0; i< numFaces * 3; i+=6) {
            tempTexils[i] = point2(0.0, 0.0);
            tempTexils[i+1] = point2(0.0, ct);
            tempTexils[i+2] =point2(ct, ct);
            tempTexils[i+3] = point2(0.0, 0.0);
            tempTexils[i+4] = point2(ct, ct);
            tempTexils[i+5] =point2(ct, 0.0);
        }
    }
    ...
    if( colorAttributes.texturized) {
		...
		glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, colorAttributes.texSize, colorAttributes.texSize, 0, GL_RGB, GL_UNSIGNED_BYTE, colorAttributes.data );
...
void Object::draw( shaderloc &prog) {
	...
	if (colorAttributes.texturized) {
        ...
        if (colorAttributes.texSize == 1200) {
            ...
            glUniform1i( prog.textureLocation, 1 );
